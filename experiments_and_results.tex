...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[r]{0.49\textwidth}
    	...
    	\vspace{-10px}
	\end{subfigure} 
    \caption{That best directions computed for each garment provided to the system. The direction with the smallest bumpiness value is shown in blue. The second best direction is shown in yellow. The bisector is shown in green. The arrows are for demonstrative purpose only, and their starting and ending point do not represent the pick and place point. The watershed computed regions are additionally overlaid upon the original image. \juansays{ocultar frase frutos secos}}
    \label{directions_several}
\end{figure}
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	~
    \begin{subfigure}[c]{\bigtablewidth}
	    \centering
    	\includegraphics[width=\textwidth]
	...
	\end{subfigure} 
    \caption[Final output of the algorithm and the computed unfolding directions.]
    {Final output of the algorithm and the computed unfolding directions. Each column includes the output corresponding to 4 of the 20 database samples for each of the 6 garment categories considered: Skirt, Jacket, Pants, Polo, Robe and Hoodie.}
    \label{fig:results}
\end{figure}
