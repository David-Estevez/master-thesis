...
\begin{figure}[thpb]
    ...
    \caption{Basic action and model-driven strategy presented by Kita \reftodo (?). The garment is held by a point with a robotic arm and different deformable models are tested against the garment data. The model that explains the best the data is selected to calculate the second grasping point. }
...
\begin{figure}[thpb]
    ...
    \caption{\warning{Sample result from experiments carried out by Li in \reftodo (?). The first 5 columns correspond to a sweater while the remaining 5 correspond to jeans. Each column correspond to a different grasping point. The first row of images shows the color images captured by the Kinect sensor, whereas the second row shows the depth images captured by the same sensor, after preprocessing. The third and fourth columns correspond to the simulation results of the garment mesh models, hanging by the ground truth and predicted points, respectively.}
    }
